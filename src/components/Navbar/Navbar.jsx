    ...
    <header className="navbar">
      <div className="navbar__left">
        ...
        <nav className="navbar__nav">
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path} className={`navbar__nav-link ${location.pathname === link.path ? "navbar__nav-link--active" : ""
              }`}>
              {link.label}
            </Link>
          ))}
      ...
      <div className={`navbar__mobile-menu ${isMobileMenuOpen ? "navbar__mobile-menu--open" : ""}`}>
        <div className="navbar__mobile-nav">
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path} className={`navbar__mobile-link ${location.pathname === link.path ? "navbar__mobile-link--active" : ""
              }`}>
              {link.label}
            </Link>
          ))}
